...
function field(x, i) {
  var f = x[i + 1];
  return call1(Stdlib_obj[1], f) ?
    caml_obj_tag(f) === Stdlib_obj[13] ?
     call2(Stdlib_printf[4], a_, f) :
     caml_obj_tag(f) === Stdlib_obj[14] ? call1(Stdlib[35], f) : cst :
    call2(Stdlib_printf[4], b_, f);
}
...
function other_fields(x, i) {
  ...
  var ae_ = field(x, i);
  return call3(Stdlib_printf[4], c_, ae_, ad_);
}
...
function fields(x) {
  ...
  if (2 < match >>> 0) {
    ...
    ab_ = field(x, 1);
    return call3(Stdlib_printf[4], d_, ab_, aa_);
  }
  switch (match) {
    case 0:
      return cst__1;
    ...
    default:
      ac_ = field(x, 1);
      return call2(Stdlib_printf[4], e_, ac_)
    }
}
  ...
  function conv(param) {
    ...
    for (; ; ) {
      ...
      if (0 === caml_obj_tag(x)) {
        constructor = x[1][1];
        Y_ = fields(x);
        return call2(Stdlib[28], constructor, Y_);
      }
      return x[1];
    }
  }
...
function print(fct, arg) {
  ...
  try {V_ = call1(fct, arg);return V_;}
    ...
    U_ = to_string(x);
    call2(Stdlib_printf[3], f_, U_);
    call1(Stdlib[63], Stdlib[40]);
...
function catch__0(fct, arg) {
  ...
  try {T_ = call1(fct, arg);return T_;}
    ...
    x = runtime["caml_wrap_exception"](x);
    call1(Stdlib[63], Stdlib[39]);
    S_ = to_string(x);
    call2(Stdlib_printf[3], g_, S_);
...
function format_backtrace_slot(pos, slot) {
  ...
  if (0 === slot[0]) {
    ...
    O_ = slot[2];
    P_ = info(slot[1]);
    return [0,call7(Stdlib_printf[4], h_, P_, O_, N_, M_, L_, K_)];
  }
  if (slot[1]) {return 0;}
  var Q_ = info(0);
  return [0,call2(Stdlib_printf[4], i_, Q_)];
}
...
function print_exception_backtrace(outchan, backtrace) {
  ...
  if (backtrace) {
    ...
    if (! (I_ < 0)) {
      ...
      for (; ; ) {
        ...
        if (match) {str = match[1];call3(Stdlib_printf[1], outchan, j_, str);}
        J_ = i + 1 | 0;
  ...
  return call2(Stdlib_printf[1], outchan, k_);
}
...
function backtrace_to_string(backtrace) {
  ...
  if (backtrace) {
    ...
    if (! (F_ < 0)) {
      ...
      for (; ; ) {
        match = format_backtrace_slot(i, caml_check_bound(a, i)[i + 1]);
        if (match) {str = match[1];call3(Stdlib_printf[5], b, l_, str);}
        G_ = i + 1 | 0;
...
module.exports = Stdlib_printexc;
