...
final class Js_of_ocaml__Sys_js {
  ...
  private static function load($joo_global_object) {
    ...
    $update_file = function($name, $content) use ($Pervasives,$caml_call1,$caml_call2) {
      $oc = $caml_call1($Pervasives[48], $name);
      $caml_call2($Pervasives[54], $oc, $content);
      return $caml_call1($Pervasives[64], $oc);
    };
    $set_channel_flusher = function($out_channel, $f) use ($caml_call1,$caml_js_wrap_callback,$runtime) {
      $f__0 = $caml_js_wrap_callback(
        function($s) use ($caml_call1,$f,$runtime) {
          return $caml_call1($f, $runtime["caml_js_to_byte_string"]($s));
        }
      );
      return $runtime["caml_ml_set_channel_output"]($out_channel, $f__0);
    };
    $set_channel_filler = function($in_channel, $f) use ($caml_js_wrap_callback,$runtime) {
      $f__0 = $caml_js_wrap_callback($f);
      return $runtime["caml_ml_set_channel_refill"]($in_channel, $f__0);
    };
    $mount = function($path, $f) use ($caml_call2,$caml_js_wrap_callback,$runtime) {
      return $runtime["caml_mount_autoload"](
        $path,
        $caml_js_wrap_callback(
          function($prefix, $path) use ($caml_call2,$f) {
            return $caml_call2($f, $prefix, $path);
          }
        )
      );
    };
    $unmount = function($path) use ($runtime) {
    ...
    $pT = function($pZ, $pY) use ($runtime) {
      return $runtime["caml_create_file"]($pZ, $pY);
    };
    $pU = function($pX) use ($runtime) {
