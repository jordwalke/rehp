...
module Share = struct
  ...
  let get
      ?(alias_strings = false) ?(alias_prims = false) ?(alias_apply = true) (_, blocks, _)
      : t =
    let count =
      Addr.Map.fold
        (fun _ block share ->
...
let runtime_fun ctx name =
  ...
  | Some runtime -> J.EAccess (J.EVar (Id.V runtime), J.EStr (name, `Utf8))
  | None -> s_var name
...
let rec translate_expr ctx queue loc _x e level : _ * J.statement_list =
  ...
  | Prim (p, l) ->
      let res =
        ...
        | Extern "caml_js_property_get", [Pv o; Pc (String f | IString f)]
          when Id.is_ident f ->
        ...
        ( J.For_statement
            ...
            , ( J.Block
                  ...
                  then (
                    if debug ()
                    then
                      Format.eprintf "@ break (%d); }@]" (Addr.Set.choose new_frontier);
