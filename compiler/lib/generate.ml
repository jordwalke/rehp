...
let s_var name = J.EVar (Id.S {Id.name=name; Id.var = None})
...
let rec constant_rec ~ctx x level instrs =
  ...
  | Tuple (tag, a) ->
    ...
    begin match detect_list 0 [] x with
      | Some elts_rev ->
        let arr,instrs =
          List.fold_left elts_rev
            ~init:([], instrs)
            ~f:(fun (arr,instrs) elt ->
            ...
            List.fold_left l
              ...
              ~f:(fun (acc,instrs) js ->
                 ...
                 | J.EStruct _ ->
                   ...
                   let instrs =
                     (J.Variable_statement [Id.V v, Some (js,Loc.N)],Loc.N) :: instrs
                   in
...
let access_queue_may_flush queue v x =
  ...
  let _,instrs,queue = List.fold_left queue
      ...
      ~f:(fun (deps,instrs,queue) ((y,elt) as eq) ->
        if Code.Var.Set.exists (fun p -> Code.Var.Set.mem p deps) elt.deps then
          (Code.Var.Set.add y deps,
           (J.Variable_statement [Id.V y, Some (elt.ce, elt.loc)], elt.loc)
           :: instrs,
           queue)
        else
...
let flush_queue expr_queue prop (l:J.statement_list) =
  let (instrs, expr_queue) =
    if prop >= flush_p then (expr_queue, []) else
      List.partition ~f:(fun (_, elt) -> should_flush prop elt.prop) expr_queue
  in
  let instrs =
...
type state =
  { all_succs : (int, Addr.Set.t) Hashtbl.t; (* not used *)
    ...
    mutable loop_stack : (Addr.t * (Javascript.Label.t * bool ref)) list;
...
module DTree = struct
  ...
  let normalize a =
    ...
    >> List.stable_sort ~cmp:(fun (_,l1) (_,l2) -> compare (List.length l1) (List.length l2))
    >> Array.of_list
  ...
  let build_switch (a : cont array) : 'a t =
    ...
    let rec loop low up =
      let array_norm : (Code.cont * int list) array = normalize (Array.sub ai ~pos:low ~len:(up - low + 1)) in
      ...
      then Branch (fst array_norm.(0))
        ...
        with Not_found ->
          ...
          if !nbcases <= m
          then
            Switch (Array.map array_norm ~f:(fun (x,l) -> l,Branch x))
          else
            let h = (up + low) / 2 in
...
let apply_fun_raw ctx f params =
  ...
  J.ECond (J.EBin (J.EqEqEq, J.EArityTest f,
           ...
           J.ECall (f, params, Loc.N),
           J.ECall (runtime_fun ctx "caml_call_gen",
                    [f; J.EArr (List.map params ~f:(fun x -> Some x))], Loc.N))

let generate_apply_fun ctx n =
  let f' = Var.fresh_n "f" in
  let f = Id.V f' in
  let params =
    Array.to_list (Array.init n ~f:(fun i ->
  ...
  J.EFun (None, f :: params,
          [J.Statement
             (J.Return_statement
                (Some (apply_fun_raw ctx f' params'))), Loc.N],
          Loc.N)
...
let _ =
  List.iter ~f:(fun (nm, nm') -> Primitive.alias nm nm')
    ["%int_mul", "caml_mul";
     "%int_div", "caml_div";
     "%int_mod", "caml_mod";
  ...
  | Apply (x, l, true) ->
    ...
    let (args, prop, queue) =
      List.fold_right
        ...
        l ~init:([], or_p px mutator_p, queue)
    in
  ...
  | Block (tag, a) ->
    let (contents, prop, queue) =
      List.fold_right
        ...
        (Array.to_list a) ~init:([], const_p, queue)
    in
    ...
    begin match ctx.Ctx.live.(Var.idx x),e with
      ...
      *)
      ...
      | _ -> flush_queue expr_queue prop
               (instrs@
                [J.Variable_statement [Id.V x, Some (ce, loc)], loc])
    end
  | Set_field (x, n, y) ->
...
and compile_decision_tree st _queue handler backs frontier interm succs loc cx dtree =
  ...
  let cx, binds = match cx with
    ...
    | _ ->
      ...
      J.EVar v, [J.Variable_statement [v,Some (cx,Loc.N)],Loc.N] in
            ...
            let (st, queue) =
              ...
              | _ -> flush_queue queue px
                       [let loc = source_location ctx pc in
                        J.Variable_statement [Id.V y, Some (cx, loc)],
                        loc]
            in
...
and compile_branch_selection pc interm =
  try
    ...
    (J.Variable_statement [Id.V x, Some (int i, Loc.N)], Loc.N) ::
...
let generate_shared_value ctx =
  ...
  then
    let applies =
      List.map (IntMap.bindings ctx.Ctx.share.Share.vars.Share.applies)
        ~f:(fun (n,v) ->
...
let f ((pc, blocks, _) as p) ~exported_runtime live_vars debug =
  let t' = Timer.make () in
  let share =
    Share.get ~alias_prims:(exported_runtime != None) p
  in
