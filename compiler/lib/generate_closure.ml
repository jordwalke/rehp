...
let add_multi k v map =
  let set =
    try Var.Map.find k map
    with Not_found -> Addr.Set.empty
  in
...
let group_closures closures =
  let names =
    List.fold_left closures
      ~init:Var.Set.empty
      ~f:(fun names x -> Var.Set.add x.f_name names)
  in
  let closures_map =
    List.fold_left closures
      ~init:Var.Map.empty
  ...
  let f free_pc blocks closures_map component =
    ...
    | SCC.Has_loop all ->
      ...
      then begin
        ...
        Format.eprintf "%s\n%!"
          (String.concat ~sep:", "
             (List.map all ~f:(fun x -> Var.to_string x)));
      end;
      let all = List.map all ~f:(fun id ->
          ...
          let blocks, free_pc =
            List.fold_left counter_and_pc
              ...
              | Let (x, Apply (f,args,true)) :: rem_rev ->
                assert (f = ci.f_name);
                let blocks =
                  Addr.Map.add direct_call_pc
                    (direct_call_block block ~counter ~x ~f:new_f ~args) blocks
                in
                let blocks =
                  Addr.Map.add bounce_call_pc
...
let rewrite_mutable free_pc blocks mutated_vars rewrite_list
  ...
  then free_pc, blocks, internal_and_external
    ...
    | _ ->
      ...
      let pcs = List.map internal_and_external ~f:(function
        | Let (_, Closure (_,(pc,_))) -> pc
        | _ -> assert false) in
      let old_xs = List.map closures_extern ~f:(function
        | Let (x, Closure _) -> x
        | _ -> assert false) in
