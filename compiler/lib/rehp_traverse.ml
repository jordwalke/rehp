...
class map : mapper = object(m)
  ...
  method source x = match x with
    ...
    | Function_declaration(id,params,body,nid) ->
      Function_declaration(m#ident id, List.map params ~f:m#ident, m#sources body,nid)
...
class map_for_share_constant = object(m)
  ...
  method expression e =
    ...
    | ECall(EVar (S {var = None; name = "require"; _}),[EStr _ ],_) ->
      e
...
class free =
  ...
  method merge_info from =
    ...
    let count = Id.IdentMap.fold (fun v k acc ->
...
class compact_vardecl = object(m)
    ...
    method private translate_st l =
      ...
      match l with
        | [] -> Empty_statement
        | x::l -> Expression_statement (
          List.fold_left l ~init:x ~f:(fun acc e -> ESeq(acc,e)))
    ...
    method merge_info from =
      super#merge_info from;
      let all = S.fold (fun e acc -> Id.IdentSet.add (V e) acc) from#state.def Id.IdentSet.empty in
      let all = StringSet.fold (fun e acc -> Id.IdentSet.add (Id.ident e) acc) from#state.def_name all in
    ...
    method private pack all sources =
      let may_flush rem vars s instr =
        if vars = []
        then rem,[],s::instr
        else rem,[],s::(Statement (Variable_statement (List.rev vars)), Loc.N)::instr in

      let rem,vars,instr =
        List.fold_left sources
          ~init:(all,[],[])
          ~f:(fun (rem,vars,instr) (s, loc) ->
            match s with
            | Statement (Expression_statement e) -> begin
                let l = m#split e in
                List.fold_left l
                  ~init:(rem,vars,instr)
                  ~f:(fun (rem,vars,instr) e -> match e with
                    | EBin(Eq,EVar id,exp) when Id.IdentSet.mem id rem ->
                      (Id.IdentSet.remove id rem,(id,Some (exp,Loc.N))::vars,instr)
                    | x -> may_flush rem vars (Statement(Expression_statement x), Loc.N) instr)
              end
      ...
      let l = List.fold_left l
          ...
          ~f:(fun acc (x, loc) ->
            ...
            | Expression_statement e ->
              ...
              let l = List.fold_left l
                  ~init:acc
                  ~f:(fun acc e -> (Expression_statement e, Loc.N)::acc) in
              l
            | _ -> (x, loc)::acc) in
      List.rev l
...
class clean = object(m)
  ...
  method sources l =
    ...
    let (st_rev,sources_rev) = List.fold_left l
        ~init:([],[])
        ~f:(fun (st_rev,sources_rev) (x, loc) ->
          match x with
