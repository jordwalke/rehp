...
/*   'program_out, */
/* ) = { */
/*   expression: ('self, 'i, Rehp.expression) => 'expression_out, */
/*   switch_case: ('self, 'i, Rehp.expression) => 'expression_out, */
/*   initialiser: */
/*     ('self, 'i, (Rehp.expression, Rehp.location)) => 'initializer_out, */
/*   statement: ('self, 'i, Rehp.statement) => 'statement_out, */
/*   statements: ('self, 'i, Rehp.statement_list) => 'statements_out, */
/*   source: ('self, 'i, Rehp.source_element) => 'source_out, */
/*   sources: ('self, 'i, Rehp.source_elements) => 'sources_out, */
/*   ident: ('self, 'i, Rehp.ident) => 'ident_out, */
/*   program: ('self, 'i, Rehp.program) => 'program_out, */
/* } */
/* constraint 'self = */
/*   tt( */
/*     'i, */
...
let map_from = (i, i_res, mapper, lst) => {
  let rec map_from_ = (i_cur, lst) =>
    switch (lst) {
    | [] => []
    | [hd, ...tl] =>
      let tl_res = map_from_(i_cur + 1, tl);
      i_cur === i ?
        [i_res, ...tl_res] :
        i_cur < i ? [hd, ...tl_res] : [mapper(hd), ...tl_res];
    };
  map_from_(0, lst);
};

/*
 * Limitation: This only works on mappers that have identical input/output
 * types. That is why it is worth exploring type safe "parallel trees" where
 * we have two trees of identical shape holding different data types, instead
 * of mapping that returns a tuple.
 */
/* let list_map_conserve = (mapper, orig_list) => { */
/*   let rec map_ = (i, lst) => */
/*     switch (lst) { */
/*     /* Never found a single difference. */ */
/*     | [] => orig_list */
/*     | [hd, ...tl] => */
/*       let hd_out = mapper(hd); */
/*       /* No difference in output */ */
/*       if (hd_out === hd) { */
/*         map_(i + 1, tl); */
/*       } else { */
/*         map_from(i, hd_out, mapper, orig_list); */
/*       }; */
/*     }; */
/*   map_(0, orig_list); */
/* }; */
 ...
 * joiner(empty, x) === x
 */
let wrap_in_struct = lst => Rehp.EStruct(lst);
let create = (empty, joiner: ('a, 'a) => 'a) => {
  open Rehp;
  let join_all = lst => List.fold_left(joiner, empty, lst);
  let empty_statement = (empty, Empty_statement);
  let debugger_statement = (empty, Debugger_statement);
  /**
  ...
  let rec fold_exprs =
    ...
    | [] =>
      ! saw_change ?
        (cur_out, orig_e) : (cur_out, wrap_res(List.rev(cur_mapped_rev)))
    | [hd, ...tl] =>
      let (out, mapped) = self.expression(self, input, hd);
      let changed = mapped !== hd;
      fold_exprs(
        saw_change || changed,
        [mapped, ...cur_mapped_rev],
        self,
        input,
        joiner(cur_out, out),
        orig_e,
    ...
    };
  {
    ...
    statements: (self, input, l) => {
      ...
      (join_all(outputs), mapped_results);
    },
    statement: (self, input, x) =>
      ...
      | Variable_statement(l) =>
        ...
        (
          List.fold_left(joiner, empty, outputs),
          Variable_statement(mapped_results),
        );
      ...
      | If_statement(e, s, sopt) =>
        ...
        let output = joiner(joiner(expr_output, if_output), sopt_output);
      ...
      | For_statement(e1, e2, e3, (s, loc)) =>
        ...
        let (s_out, s_mapped) = self.statement(self, input, s);
        let outs = joiner(joiner(joiner(e1_out, e2_out), e3_out), s_out);
        (
          outs,
      ...
      | ForIn_statement(e1, e2, (s, loc)) =>
        ...
        let (s_out, s_mapped) = self.statement(self, input, s);
        let outs = joiner(joiner(e1_out, e2_out), s_out);
      ...
      | Switch_statement(e, l, def, l') =>
        ...
        let outs = join_all([e_out, d_out, ...l_out @ l'_out]);
      ...
      | Try_statement(b, catch, final) =>
        ...
        (
          joiner(joiner(b_out, catch_out), final_out),
    ...
    expression: (self, input, x) =>
      Rehp.(
        ...
        | ESeq(e1, e2) =>
          ...
          let (e2_out, e2_mapped) = self.expression(self, input, e2);
          let joined = joiner(e1_out, e2_out);
          /* Optimized allocation */
          e1_mapped === e1 && e2_mapped === e2 ?
            (joined, x) : (joined, Rehp.ESeq(e1_mapped, e2_mapped));
        | ECond(e1, e2, e3) =>
          let (e1_out, e1_mapped) = self.expression(self, input, e1);
          let (e2_out, e2_mapped) = self.expression(self, input, e2);
          let (e3_out, e3_mapped) = self.expression(self, input, e3);
          let joined = joiner(joiner(e1_out, e2_out), e3_out);
          /* Optimized allocation */
          e1_mapped === e1 && e2_mapped === e2 && e3_mapped === e3 ?
            (joined, x) : (joined, ECond(e1_mapped, e2_mapped, e3_mapped));
        | EBin(b, e1, e2) =>
          let (e1_out, e1_mapped) = self.expression(self, input, e1);
          let (e2_out, e2_mapped) = self.expression(self, input, e2);
          let joined = joiner(e1_out, e2_out);
          /* Optimized allocation */
          e1_mapped === e1 && e2_mapped === e2 ?
            (joined, x) : (joined, Rehp.EBin(b, e1_mapped, e2_mapped));
        | EUn(b, e1) =>
          let (e1_out, e1_mapped) = self.expression(self, input, e1);
          /* Optimized allocation */
          e1_mapped === e1 ? (e1_out, x) : (e1_out, EUn(b, e1_mapped));
        | ECall(e1, e2, loc) =>
          let (e1_out, e1_mapped) = self.expression(self, input, e1);
          let (e2_outs, e2_mappeds) =
            List.split(List.map(self.expression(self, input), e2));
          (
            join_all([e1_out, ...e2_outs]),
            ECall(e1_mapped, e2_mappeds, loc),
          );
        | EAccess(e1, e2) =>
          let (e1_out, e1_mapped) = self.expression(self, input, e1);
          let (e2_out, e2_mapped) = self.expression(self, input, e2);
        ...
        | EStructAccess(e1, e2) =>
          ...
          let joined = joiner(e1_out, e2_out);
          e1_mapped === e1 && e2_mapped === e2 ?
            (joined, x) : (joined, EStructAccess(e1_mapped, e2_mapped));
        | EArrAccess(e1, e2) =>
          let (e1_out, e1_mapped) = self.expression(self, input, e1);
          let (e2_out, e2_mapped) = self.expression(self, input, e2);
          let joined = joiner(e1_out, e2_out);
          e1_mapped === e1 && e2_mapped === e2 ?
            (joined, x) : (joined, EArrAccess(e1_mapped, e2_mapped));
        | EDot(e1, id) =>
          let (e1_out, e1_mapped) = self.expression(self, input, e1);
          /* Optimized allocation */
          e1_mapped === e1 ? (e1_out, x) : (e1_out, EDot(e1_mapped, id));
        | ENew(e1, Some(args)) =>
          let (e1_out, e1_mapped) = self.expression(self, input, e1);
          let (args_outs, args_mappeds) =
            List.split(List.map(self.expression(self, input), args));
          (
            join_all([e1_out, ...args_outs]),
            ENew(e1_mapped, Some(args_mappeds)),
          );
        | ENew(e1, None) =>
          let (e1_out, e1_mapped) = self.expression(self, input, e1);
          /* Optimized allocation */
          e1_mapped === e1 ? (e1_out, x) : (e1_out, ENew(e1_mapped, None));
        | EVar(v) =>
        ...
        | EFun((idopt, params, body, gv, fv, nid)) =>
          ...
          let (sources_out, sources_mapped) =
    ...
    source: (self, input) =>
      ...
      | Function_declaration((id, params, body, gv, fv, nid)) => {
          ...
          let (sources_out, sources_mapped) =
            self.sources(self, input, body);
          let outs = join_all([ident_out, ...params_outs @ [sources_out]]);
          (
            outs,
...
let count_joiner = (cur, next) => cur + next;
