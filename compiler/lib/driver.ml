...
let generate d ~global_object_name ~exported_runtime (p,live_vars) =
  if times ()
  then Format.eprintf "Start Generation...@.";
  Generate.f p ~global_object_name ~exported_runtime live_vars d
...
let gen_missing ~global_object_name js missing =
    ...
    let miss = StringSet.fold (fun prim acc ->
        ...
        (p,
         Some (
           ECond(EBin(NotEqEq,
                      EDot(EVar (S {name=global_object_name;var=None}),prim),
...
let link ~global_object_name ~standalone ~linkall ~export_runtime js =
  ...
  let js = if Option.Optim.genprim ()
    then gen_missing ~global_object_name js missing
    else js in
  ...
  let js =
    ...
    then
      ...
      (Statement (Expression_statement(
         EBin(Eq,
              EDot(EVar (S {name=global_object_name;var=None}),"runtime"),
              EObj all))),N)
      :: js
    else js
  in
...
let pack_js ~global_object_name ~global js =
  let module J = Javascript in
  let t = Util.Timer.make () in
  if times ()
  ...
  let js =
    let f = J.EFun (None, [J.S {J.name = global_object_name; var=None }], use_strict js, J.U) in
    let expr =
      match global with
      | `Function -> f
      | `Bind_to _ -> f
...
let pack_php ~global_object_name ~global js =
  let module J = Javascript in
  let t = Util.Timer.make () in
  if times ()
  then Format.eprintf "Start Optimizing js...@.";
...
let f ?(standalone=true) ?(global=`Auto) ?(profile=o1)
      ?(dynlink=false) ~global_object_name
      ?(backend=Backend.Js) ?(linkall=false)
  ...
  print >>
  deadcode' >>
  generate d ~global_object_name ~exported_runtime >>

  link ~global_object_name ~standalone ~linkall ~export_runtime:dynlink >>

  packer ~global_object_name ~global >>
