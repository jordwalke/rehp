...
*)
  ...
  let rec need_paren l e =
    ...
    | EStruct _ | ETag _ | EVar _ | EStr _ | EBool _ | ENum _ | EQuote _ | ERegexp _| EUn _ | ENew _ ->
      false
    | EFun _ | EObj _ ->
      true
  ...
  let rec expression l f e =
    ...
    | EArr el ->
      if l > 15 then begin PP.start_group f 1; PP.string f "(" end;
      let funcIdent = S {name="ArrayLiteral";var=None} in
    ...
    | EDot (e, nm) ->
      ...
      PP.string f nm;
      (* if l > 15 then begin PP.string f ")"; PP.end_group f end *)
    | ENew (e, args) -> (*FIX: should omit parentheses when possible*)
      if l > 13 then begin PP.start_group f 1; PP.string f "(" end;
      PP.start_group f 1;
      PP.string f "new";
      PP.space f;
  ...
  and elements_as_args f l =
    match l with
      []     -> ()
    | [e] -> PP.start_group f 0; expression_or_undefined 1 f e; PP.end_group f
    | e :: r -> PP.start_group f 0; expression_or_undefined 1 f e; PP.end_group f;
      PP.string f ","; PP.break f; elements_as_args f r
  ...
  and statement ?(last=false) f (s, loc) =
    ...
    | ForIn_statement (e1, e2, s) ->
      ...
      PP.start_group f 1;
      PP.string f "(";
      expression 0 f ( ECall ((EDot (e2, "__all_enumerable_keys")), [], Javascript.N));
      PP.space f;
